#!/usr/bin/env python

"""
Implement CBC mode
https://cryptopals.com/sets/2/challenges/10
"""

from base64 import b64decode
from Crypto.Cipher import AES
from lantern.util import group
from .challenge9 import pad
from set1.challenge2 import xor


def cbc_encrypt(plaintext: bytearray, key: bytearray, iv: bytearray, block_size=AES.block_size):
    """AES CBC encrypt"""
    assert len(key) == block_size
    assert len(iv) == block_size
    plaintext = pad(plaintext, block_size)

    cipher = AES.new(key, AES.MODE_ECB)
    ciphertext = bytearray()

    previous_ciphertext = iv
    for block in group(plaintext, block_size):
        cipher_block = cipher.encrypt(bytes(bytearray.fromhex(xor(block, previous_ciphertext))))
        ciphertext += cipher_block
        previous_ciphertext = cipher_block

    return ciphertext


def cbc_decrypt(ciphertext: bytearray, key: bytearray, iv: bytearray, block_size=AES.block_size):
    """AES CBC decrypt"""
    assert len(key) == block_size
    assert len(iv) == block_size
    assert len(ciphertext) % block_size == 0

    cipher = AES.new(key, AES.MODE_ECB)
    plaintext = bytearray()

    previous_ciphertext = iv
    for block in group(ciphertext, block_size):
        plaintext_block = bytearray.fromhex(xor(cipher.decrypt(bytes(block)), previous_ciphertext))
        plaintext += plaintext_block
        previous_ciphertext = block

    return plaintext


# KEY = b"YELLOW SUBMARINE"
# IV = b"\x00" * 16
# ciphertext = cbc_encrypt(b"Tesing over 2 blocks wow paddin", KEY, IV)
# assert cbc_decrypt(ciphertext, KEY, IV) == b"Tesing over 2 blocks wow paddin\x01"

# ciphertext = ""
# ciphertext = b64decode(ciphertext)
# print(cbc_decrypt(ciphertext, KEY, IV).decode('utf-8'))
