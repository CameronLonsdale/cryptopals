...
def cbc_encrypt(plaintext: bytearray, key: bytearray, iv: bytearray, block_size=AES.block_size):
    ...
    for block in group(plaintext, block_size):
        chained_block = bytearray.fromhex(xor(plaintext, previous_ciphertext))
        previous_ciphertext = cipher.encrypt(bytes(chained_block))
...
def cbc_decrypt(ciphertext: bytearray, key: bytearray, iv: bytearray, block_size=AES.block_size):
    ...
    for block in group(ciphertext, block_size):
        ...
        plaintext += bytearray.fromhex(xor(decrypted, previous_ciphertext))
...
IV = b"\x00" * 16
ciphertext = cbc_encrypt(b"Test", KEY, IV)
assert "Test" == cbc_decrypt(ciphertext, KEY, IV).decode('utf-8')[:4]
